...
> **注**：P2P Agent 市场是 GeniusClaw OS 的核心创新，详见本节末尾的"P2P Agent 市场架构"部分。
...
### 2.7 P2P Agent 市场架构（核心创新）

#### 2.7.1 WireGuard 与 P2P 市场网络的区别
...
- **WireGuard**：用于路由出口模块（M6）的 VPN 功能
- **P2P 市场网络**：基于 libp2p，用于 Agent 间的发现与交易

#### 2.7.2 每个 Agent 的唯一标识
...
| 步骤 | 说明 |
|------|------|
| 1 | **发现**：Agent 通过 P2P 发现彼此，获取能力清单 |
| 2 | **询价**：Agent A 发送 HTTP 402 预支付请求 |
| 3 | **报价**：Agent B 返回价格（代币/数字货币） |
| 4 | **支付**：Agent A 发起支付，锁定资金 |
| 5 | **执行**：Agent B 执行服务，生成结果 |
| 6 | **确认**：Agent A 确认结果，资金释放给 B |
